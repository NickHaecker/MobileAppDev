...
class ProductsRepository {
    ...
    suspend fun getAllBuyableProduct(): List<BuyableProduct> = listOf(
        BuyableProduct(allProducts[0], 0.8),
        BuyableProduct(allProducts[1], 1.2),
        BuyableProduct(allProducts[2], 0.6),
    )

    suspend fun getProductById(id: ProductId): Product? = allProducts.firstOrNull {
        it.id == id
    }

    suspend fun getBuyableProductById(id: ProductId): BuyableProduct? = getAllBuyableProduct().firstOrNull {
        it.product.id == id
    }
}
